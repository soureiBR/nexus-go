...
func (h *GroupHandler) CreateGroup(c *gin.Context) {
	...
	// Criar grupo
	group, err := h.sessionManager.CreateGroup(req.UserID, req.Name, req.Participants)
	if err != nil {
		logger.Error("Falha ao criar grupo", "error", err, "user_id", req.UserID)
...
func (h *GroupHandler) GetGroupInfo(c *gin.Context) {
	...
	group, err := h.sessionManager.GetGroupInfo(req.UserID, req.GroupJID)
	if err != nil {
		logger.Error("Falha ao obter informações do grupo", "error", err, "user_id", req.UserID, "group_jid", req.GroupJID)
...
func (h *GroupHandler) AddParticipants(c *gin.Context) {
	...
	err := h.sessionManager.AddGroupParticipants(req.UserID, req.GroupJID, req.Participants)
	if err != nil {
		logger.Error("Falha ao adicionar participantes", "error", err, "user_id", req.UserID, "group_jid", req.GroupJID)
...
func (h *GroupHandler) RemoveParticipants(c *gin.Context) {
	...
	err := h.sessionManager.RemoveGroupParticipants(req.UserID, req.GroupJID, req.Participants)
	if err != nil {
		logger.Error("Falha ao remover participantes", "error", err, "user_id", req.UserID, "group_jid", req.GroupJID)
...
func (h *GroupHandler) PromoteParticipants(c *gin.Context) {
	...
	err := h.sessionManager.PromoteGroupParticipants(req.UserID, req.GroupJID, req.Participants)
	if err != nil {
		logger.Error("Falha ao promover participantes", "error", err, "user_id", req.UserID, "group_jid", req.GroupJID)
...
func (h *GroupHandler) DemoteParticipants(c *gin.Context) {
	...
	err := h.sessionManager.DemoteGroupParticipants(req.UserID, req.GroupJID, req.Participants)
	if err != nil {
		logger.Error("Falha ao rebaixar participantes", "error", err, "user_id", req.UserID, "group_jid", req.GroupJID)
...
func (h *GroupHandler) UpdateGroupName(c *gin.Context) {
	...
	err := h.sessionManager.UpdateGroupName(req.UserID, req.GroupJID, req.NewName)
	if err != nil {
		logger.Error("Falha ao atualizar nome do grupo", "error", err, "user_id", req.UserID, "group_jid", req.GroupJID)
...
func (h *GroupHandler) UpdateGroupTopic(c *gin.Context) {
	...
	err := h.sessionManager.UpdateGroupTopic(req.UserID, req.GroupJID, req.NewTopic)
	if err != nil {
		logger.Error("Falha ao atualizar tópico do grupo", "error", err, "user_id", req.UserID, "group_jid", req.GroupJID)
...
func (h *GroupHandler) LeaveGroup(c *gin.Context) {
	...
	// Sair do grupo
	err := h.sessionManager.LeaveGroup(req.UserID, req.GroupJID)
	if err != nil {
		logger.Error("Falha ao sair do grupo", "error", err, "user_id", req.UserID, "group_jid", req.GroupJID)
...
func (h *GroupHandler) JoinGroupWithLink(c *gin.Context) {
	...
	group, err := h.sessionManager.JoinGroupWithLink(req.UserID, req.Link)
	if err != nil {
		logger.Error("Falha ao entrar no grupo via link", "error", err, "user_id", req.UserID)
...
func (h *GroupHandler) GetGroupInviteLink(c *gin.Context) {
	...
	link, err := h.sessionManager.GetGroupInviteLink(req.UserID, req.GroupJID)
	if err != nil {
		logger.Error("Falha ao obter link de convite", "error", err, "user_id", req.UserID, "group_jid", req.GroupJID)
...
func (h *GroupHandler) RevokeGroupInviteLink(c *gin.Context) {
	...
	link, err := h.sessionManager.RevokeGroupInviteLink(req.UserID, req.GroupJID)
	if err != nil {
		logger.Error("Falha ao revogar link de convite", "error", err, "user_id", req.UserID, "group_jid", req.GroupJID)
